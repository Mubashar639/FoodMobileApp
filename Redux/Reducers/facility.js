...
const Facility = (
  ...
  switch (action.type) {
    ...
    case actionTypes.FACILITY_REMOVE:
      debugger;
      {
        const facilities = state.facilities.filter(
          (facility, index) => facility._id !== action.facility
        );
        return {
          ...state,
          isLoading: false,
    ...
    case actionTypes.FACILITY_FILTER:
      debugger;
      {
        const facilities = state.facilities.filter(
          (facility, index) =>
            facility.name.toLowerCase().indexOf(action.facility) !== -1
        );
    ...
    case actionTypes.FACILITY_UPDATE:
      debugger;
      {
        const facilities = state.facilities.map((facility, index) => {
          if (facility._id === action.facility.id)
            return action.facility.updatedfacility;
